package cn.vision.net
{
	...
	public final class FTPLoader2 extends VSEventDispatcher implements ITimeout
	{
		...
		public function FTPLoader2($request:FTPRequest = null)
		{
			super();
		...
		{
			return !StringUtil.empty($item.substr(0, 3));
		}
...
class CtrlSocket extends Socket
{
	
	public static function createSocket(request:FTPRequest, funcObj:Object = null):CtrlSocket
	{
		var requestDict:Object = getDict(request);
		var socket:CtrlSocket = (requestDict.idle.length > 0)
			? requestDict.idle.pop()
			: new CtrlSocket;
		
		if (funcObj)
		{
			for (var key:String in funcObj)
			{
				if (funcObj[key] && funcObj[key] is Function)
					socket.addEventListener(key, funcObj[key]);
			}
		}
		
		socket.using = true;
		
		if (requestDict.using.indexOf(socket) == -1)
		{
			requestDict.using.push(socket);
		}
	...
	public static function removeSocket(socket:CtrlSocket):void
	{
		
	}
	
	private static function getDict(request:FTPRequest):Object
	{
		var key:String = request.host + "-" + request.port + "-" + request.userName + "-" + request.passWord;
		DICT[key] = DICT[key] || {using:[], idle:[]};
		return DICT[key];
	}
